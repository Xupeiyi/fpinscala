...
```
We can use this `flatMap` to implement a new version of `listOfN`, whose 
...
Now we can go back to `Prop`, which currently is:
```scala worksheet
trait Prop:
...
```
It might feel a bit unnatural to use the `None` case of `Option` to indicate
success. We can make a new data type to show our intents more clearly:
```scala worksheet
enum Result:
  case Passed
...
```
We still cannot generate a `Prop` by passing a `Gen` to `Prop.forAll`. It needs
an `RNG` to pass to `Gen`:
```scala worksheet
opaque type Prop = (TestCases, RNG) => Result
```
For other dependencies, we can add them as extra parameters to `check` later.  
Now we can implement `forAll`, `&&` and `||`:
```scala worksheet
